				...
				while (event != XmlPullParser.END_DOCUMENT) {
					...
					case XmlPullParser.START_TAG:
						if ("AVTransportURI".equals(parser.getName())) {
							if(TextUtils.isEmpty(parser.getAttributeValue(0))) {//解决ios同时推送状态监听问题
								stopPlay();
								stopAutoGetProgress();
								progressExecutor.shutdown();
								if(listener != null) {
									listener.onError("推送失败");
								}
							}
							Log.e("parseXML", "getAttributeValue->"+parser.getAttributeValue(0));
							return;
						}
						break;
					}
