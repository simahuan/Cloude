...
public abstract class TransferTask implements Runnable {
		...
		synchronized (mInfo) {
			...
			if (mInfo.takeControl == TransferControl.PAUSE) {
				...
				if (mInfo.storageDir != null) {
					fileDelete(mInfo.storageDir + mInfo.filename);
				}
	...
	protected void fileDelete( final String destionPath){
		new Handler().postDelayed(new Runnable() {
			
			@Override
			public void run() {
				try {
					LogCat.e("========doInbackgroud begin delete====================" + destionPath);
					sardine.delete(destionPath);
				} catch (Exception e) {
					LogCat.e("=========fileDelete===Exception=================");
					e.printStackTrace();
				}
			}
		}, 500);
...
//				} else {
//					UIHelper.showToast(mContext, "删除失败");
//				}
//			}
//		});
	}
