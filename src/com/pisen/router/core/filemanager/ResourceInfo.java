...
import com.pisen.router.core.image.ImageViewer;
...
public class ResourceInfo implements Serializable {
	...
	public static void doOpenFile(Context context, ResourceInfo resource, List<ResourceInfo> resourceList) {
		try {
			String path = resource.path;// URLUtils.encodeURL(resource.path);
			...
			case Image:
				resourceList = getFileTypeResourceList(resourceList, fType);
				ImageViewer.start(context, resource, resourceList);
				break;
			case Audio: {
				resourceList = getFileTypeResourceList(resourceList, fType);
