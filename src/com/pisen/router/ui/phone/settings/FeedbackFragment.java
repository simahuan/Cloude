	...
	public void submitClick() {
		try {
			...
			if (feedbackContact == null || feedbackContact.trim().equals("")) {
				...
				if (checkMobileNumber(feedbackContact)) {
					submit();
				} else if (checkEmail(feedbackContact)) {
					submit();
				} else {
					UIHelper.showToast(activity, "您输入的手机或邮箱格式不正确");
				}
//				if (ValidatorUtils.isMobilePhone(feedbackContact)||ValidatorUtils.isPhone(feedbackContact)) {
//					submit();
//				} else if (ValidatorUtils.isEmail(feedbackContact)) {
//					submit();
//				} else {
//					UIHelper.showToast(activity, "您输入的手机或邮箱格式不正确");
//				}
	...
	private void submit() throws JSONException {
		...
		request.execute(new String[] { "", "正在提交，请稍候..." }, HttpKeys.FEED_BACK_URL, obj.toString(), new OnHttpCallBack() {
			...
			public void getHttpResult(String result) {
				...
				UIHelper.showToast(activity, "发送成功，谢谢您的意见反馈");
				return;
			}
		});
	}
