...
//	}
	...
	private void initItemView(ViewHolder holder, TransferInfo info) {
		if (info.ctag.value.equals(TransferCTag.FlashRecv.value)) {
			...
			/*处理设备名称*/
			if(infos.length >0) {
				holder.fromView.setText(String.format("来自%s的%s", info.remoteHostName, infos[0]));
			}
			//设置头像
			if(infos.length >1) {
				holder.headerView.setImageResource(IconResource.getIcon(Integer.parseInt(infos[1])));
			}else {
				holder.headerView.setImageResource(IconResource.getIcon(-1));
			}
		}else {
			holder.headerView.setImageResource(IconResource.getIcon((PreferencesUtils.getInt(KeyUtils.NICK_HEAD, -1))));
		}
