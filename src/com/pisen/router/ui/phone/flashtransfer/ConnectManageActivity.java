...
public class ConnectManageActivity extends Activity implements OnClickListener, OnDeviceSelectListener, OnScanResultCallback {
	...
	//用户正在连接ap
	private static final int WHAT_USER_CONNECTING = 7;
	//用户连接ap失败
	...
	private void startInitAnim() {
		Animation scaleAnim  = new ScaleAnimation(0.2f, 1f, 0.2f, 1f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
		...
		searchButton.setAnimation(scaleAnim);
		scaleAnim.startNow();
...
//			}
	...
	private Timer waitConnectTimer;
	private static final int COUNT_WAIT_CONNECT = 30;

//	/**
//	 * host扫描是否有客户端连接
//	 */
//	private void queryDeviceConnected() {
//		if (waitConnectTimer == null) {
//			waitConnectTimer = new Timer();
//		} else {
//			waitConnectTimer.cancel();
//		}
//		
//		TimerTask task = new TimerTask() {
//			@Override
//			public void run() {
//				transferNetUtil.getApClientList(true, new FinishScanListener() {
//
//					@Override
...
//		};
//
//		waitConnectTimer.schedule(task, 2000, 2000);
//	}
	 ...
	 * 显示AP的客户端正在连接ap视图
	 */
//	private void showApConnectingView() {
//		createApLayout.showConnectingDevice();
//	}
	...
	private void showSearchFailedView() {
		if(searchDeviceView.getVisibility() == View.VISIBLE && searchDeviceView.getDeviceCount() <=0) {
			if(DEBUG) Log.e(TAG, "===showSearchFailedView===");
//			wifiManager.setWifiEnabled(false);
	...
	private void showConnectDeviceFaildView() {
		if(DEBUG) Log.e(TAG, "===showConnectDeviceFaildView===");
		searchDeviceView.setVisibility(View.VISIBLE);
		createApLayout.setVisibility(View.GONE);
...
//		wifiManager.setWifiEnabled(false);
	...
	private void showConnectingDeviceView(TransferDevice device) {
		if(DEBUG) Log.e(TAG, "===showConnectingDeviceView===");
		searchDeviceView.stopScan();
//		searchDeviceView.showTips("努力连接中...");
	 ...
	 * 显示连接设备成功视图
	 */
	private void showConnectDeviceSucceedView() {
		if(DEBUG) Log.e(TAG, "===showConnectDeviceSucceedView===");
		searchDeviceView.stopScan();
		searchDeviceView.showTips("连接成功");
		refreshButton.setVisibility(View.VISIBLE);
	}
