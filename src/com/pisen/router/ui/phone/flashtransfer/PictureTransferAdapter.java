...
public class PictureTransferAdapter extends TimeAxisSectionAdapter implements  OnClickListener, OnLongClickListener {
	...
	protected void diplayImage(String imagePath, final ImageView imageView) {
		...
		Picasso.with(getContext()).load(new File(imagePath)).placeholder(R.drawable.thumbnail_pic).error(R.drawable.thumbnail_pic_fail).centerInside().resize(itemImageLayoutParams.width*2, itemImageLayoutParams.height*2).error(R.drawable.thumbnail_pic_fail).into(imageView);
	}
